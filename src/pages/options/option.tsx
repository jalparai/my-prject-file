...
import { getAllCategories, getAllOptions, updateOptionSettingAPI } from "./api";
...
const Options = () => {
  ...
  const handleUpdateOption= async (id: string, status: boolean) => {

    await updateOptionSettingAPI(id, status);
    await getAllOptions(setData);

  }

  useEffect(() => {
    getAllOptions(setData);
  ...
  return (
    <>
      <div>
        ...
        <Optiontable data={data} handleUpdateOption={handleUpdateOption}/>
